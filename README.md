...
1. **Форкните или импортируйте репозиторий на Replit.**
2. В разделе Secrets (Environment Variables) добавьте:
   - `BOT_TOKEN` — токен вашего Telegram-бота
   - `MAIN_ADMIN_ID` — ваш Telegram user ID (числом)
   ...
   ```
3. Убедитесь, что в файле `.replit` указано:
   ```
   run = "python main.py"
   language = "python3"
   ```
4. Все зависимости установятся автоматически из `requirements.txt`.
5. Нажмите Run — бот запустится и будет работать в облаке!

**Важно:**
- Для работы с БД используется файл `military_tracker.db` (создаётся автоматически).
- Для экспорта нужен пакет `openpyxl` (уже в requirements.txt).
- Время напоминаний и другие настройки можно менять через меню бота.
